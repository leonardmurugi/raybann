...
router.get("/lands", authenticateToken, async (req, res) => {
    try {
        const result = await pool.query(`
...
router.get("/approvals/pending", authenticateToken, async (req: any, res: any) => {
...
router.post("/approvals/payment/:id", authenticateToken, async (req: any, res: any) => {
    if (req.user.role !== 'admin') return res.status(403).json({ error: "Unauthorized" });
    try {
...
router.post("/approvals/expense/:id", authenticateToken, async (req: any, res: any) => {
    if (req.user.role !== 'admin') return res.status(403).json({ error: "Unauthorized" });
    try {
